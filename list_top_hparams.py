...
# LICENSE file in the root directory of this source tree.# Copyright (c) Facebook, Inc. and its affiliates. All Rights Reserved
...
if __name__ == "__main__":
    ...
    parser.add_argument('--shuffle_ctxt', type=int, default = 0, required=True)
    args = parser.parse_args()

    records = utils.load_records(args.input_dir)
    print("Total records:", len(records))

    records = utils.get_grouped_records(records)
    records = records.filter(
        lambda r:
            r['dataset'] == args.dataset and
            r['algorithm'] == args.algorithm and
            r['shuffle_ctxt'] == args.shuffle_ctxt
    )
